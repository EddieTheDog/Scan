  ...
  async fetch(req, env) {
    ...
    if (url.pathname === "/submit" && req.method === "POST") {
      ...
      await env.DB.prepare(`
        INSERT OR IGNORE INTO checkins (qr_value, name, seat, status, scanned_at)
        VALUES (?, ?, ?, 'pending', ?)
      `).bind(qr, name, seat || "", new Date().toISOString()).run();
      return json({ ok: true });
    }
    ...
    if (url.pathname === "/list") {
      const { results } = await env.DB.prepare(`
        SELECT * FROM checkins WHERE status = 'pending'
